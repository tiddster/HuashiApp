...
public class MyCookieJar implements CookieJar {
    ...
    public void saveFromResponse(HttpUrl url, List<Cookie> cookies) {
        Logger.i(TAG+" saveFromResponse:  host   " + url.host()+" "+cookies.size());
        manager.addAll(url.host(), cookies);


        //这个是为了和之前的请求课表的方法兼容，而增加的，未来后端会改进
        for (int i = 0; i < cookies.size(); i++) {
            Logger.i(TAG+" saveFromResponse: " + cookies.get(i).name() + "  " + cookies.get(i).value());
    ...
    public List<Cookie> loadForRequest(HttpUrl url) {
        Logger.i(TAG+" loadForRequest:  " + url.host());
        List<Cookie> cookies = manager.provideCookies(url.host());
        Logger.i(TAG+" loadForRequest: " + cookies.size());



        for (int i = 0; i < cookies.size(); i++) {
            Cookie cookie=cookies.get(i);
            Logger.i(TAG+" loadForRequest: " + cookies.get(i).name() + "  " + cookies.get(i).value()+" "+cookies.get(i).expiresAt()+" "+cookie.secure()+" "+cookie.persistent()+" "+cookie.path()+" "+cookie.domain()+" "+cookie.httpOnly()+" "+cookie.hostOnly());
        }
    ...
    public void clearCookie() {
        Logger.i(TAG+" clearCookie: clear");
        manager.clearAll();
    }
